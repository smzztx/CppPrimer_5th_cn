...
《C++ Primer习题集》上给出的答案为vector更优，原文如下：“ 对这个问题而言，vector 更好。因为，虽然vector 不会维护元素值的序，set 会维护关键字的序，但注意到，我们是逐行读取输入文本的，因此每个单词出现的行号是自然按升序加入到容器中的，不必特意用关联容器来保证行号的升序。而从性能角度，set是基于红黑树实现的，插入操作时间复杂性为0(log n) (n为容器中元素数目)，而vector的 push back 可达到常量时间。
